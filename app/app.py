...
def map_view():
    ...
    station_gdf = gpd.read_file("../data/station_stop/stationstop.shp")
    ...
    for idx, row in station_gdf.iterrows():
        folium.CircleMarker(
            location=[row.geometry.coords[0][1], row.geometry.coords[1][0]], # ここの部分でエラーが出るので，確認をする．
            radius=3,
            color="blue",
            fill=True,
            fill_opacity=0.7
    ...
    return render_template("map.html")  # map 引数は不要
